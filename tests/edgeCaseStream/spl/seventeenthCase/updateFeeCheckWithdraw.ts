...
const secondStreamAmount = new anchor.BN(2000000);
describe("zebec token updateFeeCheckWithdraw", () => {
  ...
  it("Second Token Stream", async () => {
    ...
    assert.equal(data_account.amount.toString(), secondStreamAmount.toString());
    assert.equal(data_account.sender.toString(), sender.publicKey.toString());
    assert.equal(
      data_account.receiver.toString(),
      receiver.publicKey.toString()
  ...
  it("Withdraw Second Token Stream", async () => {
    ...
    let expectedWithdrawAmount =
      parseInt(perSecondAmount.toString()) * totalSpendTime;
    let zebecFee = parseInt(((expectedWithdrawAmount / 100) * 0.3).toString());

    let expectedZebecVaultBalance =
      zebecVaultBalanceBeforeWithdraw - expectedWithdrawAmount;
    let expectedReceiverBalance =
      Number(receiverBalanceBeforeWithdraw) +
      Number(expectedWithdrawAmount) -
      Number(zebecFee);
