...
const streamedAmount = new anchor.BN(2000000);
describe("zebec token startStreamInstantTransferError", () => {
  ...
  it("Token Deposit", async () => {
    ...
    const depositedAmount = new anchor.BN(3000000);
    const tx = await zebecProgram.rpc.depositToken(depositedAmount, {
      accounts: {
    ...
    assert.equal(tokenbalance.toString(), depositedAmount.toString());
  });
  it("Token Stream", async () => {
    ...
    assert.equal(data_account.amount.toString(), streamedAmount.toString());
    ...
    assert.equal(withdraw_info.amount.toString(), streamedAmount.toString());
  });
